  ...
  private static int build(
    ...
    for (Map.Entry<PathFragment, ActionInput> e : inputs.entrySet()) {
      ...
      switch (metadata.getType()) {
        ...
        case SYMLINK:
          throw new IllegalStateException(String.format("Encountered symlink input '%s', but all"
              + " symlinks should have been resolved by SkyFrame. This is a bug.", path));

        case SPECIAL_FILE:
          throw new IOException(String.format("The '%s' is a special input which is not supported"
              + " by remote caching and execution.", path));
  ...
  private static void explodeDirectory(PathFragment dirname,
    ...
    for (Dirent entry : entries) {
      ...
      switch (entry.getType()) {
        ...
        case SYMLINK:
          throw new IllegalStateException(String.format("Encountered symlink input '%s', but all"
              + " symlinks should have been resolved by readdir. This is a bug.", path));
